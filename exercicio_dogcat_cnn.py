...
# A base de dados DogsAndCats tem 2 classes de objetos!!!!
n_classes = 2
nepochs = 10                      # Numero de epocas para o treinamento!!!
batch_size = 32                   # Numero de imagens por batch!!!
# Todas as imagens devem ser redimensionadas para 32x32 pixels!!!
image_size = 32
...
def convolutional_neural_network(x, prob):
    ...
    fc1 = tf.contrib.layers.fully_connected(
        inputs=flat, num_outputs=16, activation_fn=tf.nn.relu)

    fc1 = tf.nn.dropout(fc1, prob)

    out = tf.contrib.layers.fully_connected(
        inputs=fc1, num_outputs=n_classes, activation_fn=None)
...
def read_dataset(filename):
    ...
    for img in tqdm(os.listdir(filename)):
        ...
        # deixa com as dimensões definidas
...
# Funções de custo:
error1 = tf.reduce_mean(-tf.reduce_sum(Y *
                                       tf.log(tf.nn.softmax(Ypred)), reduction_indices=[1]))
error2 = tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(logits=Ypred, labels=Y))
error3 = tf.reduce_mean(tf.reduce_sum(
    tf.square(tf.nn.softmax(Ypred) - Y), reduction_indices=[1]))
error = error2

# Funções para minimização de erro:
optimizer1 = tf.train.GradientDescentOptimizer(
    learning_rate=learning_rate).minimize(error)
...
with tf.Session() as sess:
    ...
    for epoch in range(nepochs):
        ...
        for i in range(total_batch):
            ...
            err, acc = sess.run([error, accuracy], feed_dict={
                                X: batch_xs, Y: batch_ys, prob: kprob})
            train_err += err
            train_acc += acc
            train_batches += 1
